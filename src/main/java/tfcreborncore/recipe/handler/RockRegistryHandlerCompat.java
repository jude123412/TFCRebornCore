    ...
    public void registerForestryRecipes(FMLPostInitializationEvent r) {
        for (Rock rock : TFCRegistries.ROCKS) {
            ForestryRecipeManager.addMoistenerRecipe(
                    RecipeHelper.getItemStack(Mods.TERRAFIRMACRAFT.ID, "bricks/" + rock.getRegistryName().getPath().toLowerCase()),
                    RecipeHelper.getItemStack(Mods.TFC_DECORATION.ID, "mossy_bricks/" + rock.getRegistryName().getPath().toLowerCase()),
                    8
            );
        }
    }
